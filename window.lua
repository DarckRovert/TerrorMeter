...
local internals = TerrorMeter.internals

local textures = TerrorMeter.textures
local spairs = TerrorMeter.spairs
local round = TerrorMeter.round
...
local function CreateBar(parent, i, background)
  parent.bars[i] = parent.bars[i] or CreateFrame("StatusBar", "TerrorMeterBar" .. i, parent)
  ...
  parent.bars[i].lowerBar = parent.bars[i].lowerBar or CreateFrame("StatusBar", "TerrorMeterLowerBar" .. i, parent)
...
local function CreateWindow(wid)
  ...
  frame.btnMode = CreateFrame("Button", "TerrorMeterDamage", frame)
  ...
  frame.btnMode.caption = frame.btnMode:CreateFontString("TerrorMeterTitle", "OVERLAY", "GameFontWhite")
  ...
  for name, template in pairs(menubuttons) do
    frame["btn"..name] = CreateFrame("Button", "TerrorMeter" .. name, frame)
    ...
    button.caption = button:CreateFontString("TerrorMeter"..name.."Title", "OVERLAY", "GameFontWhite")
  ...
  frame.btnAnnounce = CreateFrame("Button", "TerrorMeterReset", frame)
  ...
  frame.btnAnnounce.tex:SetTexture("Interface\\AddOns\\TerrorMeter" .. (tbc and "-tbc" or "") .. "\\img\\announce")
  ...
  frame.btnAnnounce:SetScript("OnClick", function()
    if IsShiftKeyDown() then
      ...
      local dialog = StaticPopupDialogs["TERRORMETER_QUESTION"]
      dialog.text = text

      dialog.OnAccept = function() frame:Refresh(nil, true) end
      StaticPopup_Show("TERRORMETER_QUESTION")
    end
  end)

  frame.btnSettings = CreateFrame("Button", "TerrorMeterReset", frame)
  ...
  frame.btnSettings.tex:SetTexture("Interface\\AddOns\\TerrorMeter" .. (tbc and "-tbc" or "") .. "\\img\\settings")
  ...
  frame.btnSettings:SetScript("OnClick", function()
    if TerrorMeter.settings:IsShown() then
      TerrorMeter.settings:Hide()
    else
      TerrorMeter.settings:Show()
    end
  end)

  frame.btnReset = CreateFrame("Button", "TerrorMeterReset", frame)
  ...
  frame.btnReset.tex:SetTexture("Interface\\AddOns\\TerrorMeter" .. (tbc and "-tbc" or "") .. "\\img\\reset")
  ...
  frame.btnWindow = CreateFrame("Button", "TerrorMeterReset", frame)
  ...
  if frame:GetID() == 1 then
    frame.btnWindow.tex:SetTexture("Interface\\AddOns\\TerrorMeter" .. (tbc and "-tbc" or "") .. "\\img\\plus")
    ...
    frame.btnWindow:SetScript("OnClick", function()
      for i=1,10 do
        if not TerrorMeter.window[i] then
          TerrorMeter.window[i] = CreateWindow(i)
          TerrorMeter.window.Refresh(true)
          return
        end
      end
    end)
  else
    frame.btnWindow.tex:SetTexture("Interface\\AddOns\\TerrorMeter" .. (tbc and "-tbc" or "") .. "\\img\\minus")
    frame.btnWindow.tooltip = {
      "Remove Window",
  ...
  frame.btnResize.tex:SetTexture("Interface\\AddOns\\TerrorMeter" .. (tbc and "-tbc" or "") .. "\\img\\resize")
  ...
  frame.border = CreateFrame("Frame", "TerrorMeterBorder", frame)
